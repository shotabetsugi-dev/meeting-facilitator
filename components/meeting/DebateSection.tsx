...
import type { Debate, Meeting } from '@/types'
...
export function DebateSection({ meetingId, meeting }: DebateSectionProps) {
          ...
          <div className="space-y-4">
            {isDraftMode ? (
              ...
              <div className="space-y-3 p-4 bg-[var(--background)] rounded-lg border border-[var(--card-border)]">
                <div>
                  <h4 className="text-xs font-semibold text-[var(--accent-purple)] mb-1">テーマ</h4>
                  <p className="text-sm text-[var(--foreground)]/80">{localDebate.theme || '未設定'}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h4 className="text-xs font-semibold text-[var(--accent-purple)] mb-1">賛成派</h4>
                    <p className="text-sm text-[var(--foreground)]/80">{localDebate.pro_side || '未設定'}</p>
                  </div>
                  <div>
                    <h4 className="text-xs font-semibold text-[var(--accent-purple)] mb-1">反対派</h4>
                    <p className="text-sm text-[var(--foreground)]/80">{localDebate.con_side || '未設定'}</p>
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-semibold text-[var(--accent-purple)] mb-1">メモ</h4>
                  <p className="text-sm text-[var(--foreground)]/80 whitespace-pre-wrap">
                    {localDebate.memo || '未設定'}
                  </p>
                </div>
              </div>
            )}
